...
- Não executar comandos Git mutáveis, salvo quando solicitado pelo usuário;
- Não remover ou modificar manualmente o diretório `.git/`;
- Permitir comandos Git estritamente read-only quando necessários para identificar o estado, a baseline de uma etapa, o histórico ou o delta de uma execução autorizada;
- Considerar como comandos Git read-only permitidos, quando aplicáveis: `git status`, `git status --short`, `git diff`, `git diff --check`, `git ls-files`, `git log`, `git show`, `git rev-parse` e `git cat-file`;
- Não executar `commit`, `add`, `reset`, `restore`, `clean`, `checkout`, `merge`, `rebase`, `cherry-pick`, `tag`, `push`, `pull`, `fetch` ou `stash`, salvo se solicitado;
- Quando um commit for solicitado, seguir o padrão Conventional Commits e preservar a convenção específica do projeto quando ela estiver definida;
- Inspecionar o estado e a baseline Git antes de gerar artefatos de diff de fases/etapas;
- Não criar commits intermediários apenas para facilitar geração de diff, reconstrução de baseline ou diagnóstico;

### Baseline Git e geração de diff por etapa

Quando o projeto usar fases ou etapas formais, o histórico Git deve ser a fonte preferencial para delimitar o delta entre etapas.

Fluxo esperado:

```text
Último estado aceito antes da nova etapa
        ↓
Commit de baseline
        ↓
Nova etapa em execução
        ↓
Diff contra a baseline
        ↓
Revisão e aceite
        ↓
Commit da etapa atual
        ↓
Possíveis commits aceitos entre etapas
        ↓
Último commit aceito vira a baseline da próxima etapa
```

Regras:

- Antes de iniciar uma nova etapa formal, identificar por comandos Git read-only o **último commit aceito existente imediatamente antes do início da etapa**;
- Usar esse commit como baseline oficial da nova etapa;
- Normalmente a baseline corresponderá ao commit de encerramento da etapa anterior, mas pode corresponder a um commit posterior de manutenção, documentação, processo, configuração ou outro ajuste deliberadamente aceito entre etapas;
- Mudanças realizadas entre etapas devem possuir commit próprio quando forem permanentes e não devem ser incorporadas silenciosamente ao diff da etapa seguinte;
- Se existir um ou mais commits aceitos entre o encerramento da etapa anterior e o início da nova etapa, usar o último desses commits como baseline;
- Gerar o diff da etapa atual antes do commit da própria etapa;
- Para arquivos rastreados, preferir `git diff <commit-de-baseline> -- <escopo-da-etapa>`;
- Para arquivos novos ainda não rastreados, identificá-los por comandos read-only e incluí-los no artefato como diff unificado contra `/dev/null`, sem executar `git add` apenas para facilitar a geração do patch;
- Para arquivos removidos, preservar a representação de deleção produzida pelo Git;
- O arquivo `*_DIFF.diff` deve conter somente o delta entre o commit de baseline e o estado final da etapa atual;
- Não repetir no diff alterações já consolidadas na baseline;
- Após o aceite formal da etapa, o commit criado pelo usuário ou explicitamente autorizado torna-se o estado aceito mais recente; se não houver outro commit deliberado entre etapas, ele será a baseline da etapa seguinte;
- Se o commit de baseline não puder ser identificado com segurança, registrar a divergência antes de prosseguir;
- Não reconstruir a baseline por aplicação de patches quando existir um commit Git que represente corretamente o último estado aceito;
- Tratar `interdiff`, aplicação encadeada de patches, reconstrução histórica e clones temporários apenas como mecanismos excepcionais de recuperação quando não existir uma baseline Git confiável;
- Validar o diff quanto a arquivos esperados, hunks completos, contagens coerentes, newline final, ausência de truncamento e ausência de arquivos fora do escopo antes de publicá-lo;
