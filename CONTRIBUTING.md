...
Instale os pré-requisitos descritos no [INSTALL.md](INSTALL.md), incluindo Docker Engine, Docker Compose Plugin, Git e GNU Make. O usuário atual também deve possuir permissão para executar o Docker.
...
O ambiente de desenvolvimento combina o código da aplicação, os serviços definidos pelo Docker Compose e os arquivos locais `.env` e `.env.dev`. Mantenha as configurações de desenvolvimento separadas das definições de produção e use dados descartáveis nas validações.

### Artefatos de desenvolvimento na raiz do projeto

Os arquivos mantidos na raiz concentram a configuração dos ambientes, a automação, a construção das imagens e a documentação necessária à adaptação do template.
...
- Banco (`db_name`): Use `<nome-aplicacao>-<cliente>-<ambiente>`, como `minhaapp-contoso-prd`;
...
```text
feat(compose): adiciona um serviço auxiliar
fix(entrypoint): preserva os arquivos gerados pela aplicação
docs: detalha a preparação do ambiente
build(dockerfile): atualiza a imagem base
```
