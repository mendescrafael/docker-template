...
Este documento descreve o ambiente de desenvolvimento do projeto Docker Template, a finalidade dos arquivos auxiliares presentes na raiz do projeto e os comandos usados para validar uma contribuição.
...
Ao propor uma mudança, preserve o objetivo do projeto: oferecer uma base genérica, modular e parametrizável, que possa ser adaptada a diferentes aplicações sem duplicar a infraestrutura.
...
Instale os pré-requisitos descritos no `INSTALL.md`, incluindo Docker Engine, Docker Compose Plugin, Git e GNU Make. O usuário atual também deve possuir permissão para executar o Docker.

Faça um fork do repositório, clone-o e crie uma branch para a contribuição:

```bash
git clone https://github.com/SEU-USUARIO/docker-template.git
cd docker-template
...
- Mantenha no estágio `BASE` somente recursos compartilhados, no `DEV` as ferramentas de desenvolvimento e no `PRD` as configurações de produção.
...
- Ao criar ou alterar uma variável, atualize os arquivos `.env.example` e `.env.dev.example` aplicáveis e a documentação.
...
- Mantenha `data/utils/app-entrypoint` compatível com as variáveis e os templates do projeto.
...
- Use exemplos genéricos, reproduzíveis e sem informações sensíveis.
...
- Arquivos de `data/misc/` ou outros artefatos locais;
- Logs que exponham informações confidenciais.

Antes de criar um commit, revise as alterações com `git diff` e confirme que apenas os arquivos pretendidos serão enviados. Caso um relato de erro precise de logs ou configurações, remova ou substitua todos os dados sensíveis.
...
Não é necessário implantar em produção para validar uma contribuição. Não execute `make clean`, `make prune-volumes` ou outros comandos de limpeza sem antes verificar os recursos Docker do hospedeiro, pois eles podem afetar outros projetos.

Se alguma verificação não puder ser executada, explique o motivo e informe no pull request quais validações foram realizadas. Mudanças específicas da aplicação também devem executar os respectivos testes, linters ou verificações adicionais.
...
- Nenhuma credencial, chave privada, dump ou arquivo local foi incluído;
- Os arquivos `.example` e a documentação foram atualizados quando necessário;
- Os novos alvos do Makefile aparecem em `make help`;
- A contribuição não introduz valores específicos de uma aplicação no núcleo genérico sem justificativa.
...
- Sistema operacional e versões do Docker, Docker Compose e GNU Make;
...
- Logs necessários, devidamente anonimizados.

Antes de relatar o problema, consulte as seções de diagnóstico do `INSTALL.md` e verifique se já existe uma issue equivalente.
